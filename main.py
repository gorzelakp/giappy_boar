...
def score_display(game_state):
    if game_state == 'main_game':
        ...
        high_score_surface = game_font.render(f'Top: {int(high_score)}', True, (255, 255, 255))
...
game_over_surface = pygame.image.load('assets/giappy.png').convert_alpha()
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_SPACE and game_active:
                bird_movement = 0 # ponieważ grawitacja cały czas się zwiększa
                bird_movement -= 12 
                flap_sound.play()
